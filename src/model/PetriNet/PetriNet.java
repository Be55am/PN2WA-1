...
public class PetriNet implements Serializable{
    ...
    public PetriNet(Graph graph,String netName){
        ...
        model.MCGGenerator.Transition[] transitions=new Transition[graph.getTransitions().size()];
        String[] places=new String[graph.getPlaces().size()];
        int[][] valuesPre=new int[graph.getPlaces().size()][graph.getTransitions().size()];
        model.MCGGenerator.Place[] initialPlaces=new Place[places.length];

        for (int i = 0; i <graph.getTransitions().size(); i++) {
            String name=graph.getTransitions().get(i).getName();
            Event e=new Event(graph.getTransitions().get(i).getEvent());
            Transition t=new Transition(name,e);
            transitions[i]=t;
        }
        ...
        for (int i = 0; i <graph.getPlaces().size() ; i++) {
            ...
            Place p=new model.MCGGenerator.Place(graph.getPlaces().get(i).getName(),m);
            initialPlaces[i]=p;
        }
        ...
        this.initialMarking=new Node("M0",initialPlaces);
